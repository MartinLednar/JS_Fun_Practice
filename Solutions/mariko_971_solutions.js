...
const compose =
  (...fn) =>
  (...x) =>
    fn.reduce((ac, el) => el(ac), fn.shift()(...x));
...
function liftmbM(binary, op) {
  return (m1, m2) => m(binary(m1.value, m2.value), `(${m1.source}${op}${m2.source})`);
}
...
function liftmb(binary, op) {
  return (m1, m2) => {
    ...
    typeof m1 === "object" ? ((val1 = m1.value), (src1 = m1.source)) : typeof m1 === "number" ? ((val1 = m1), (src1 = m1)) : null;
    typeof m2 === "object" ? ((val2 = m2.value), (src2 = m2.source)) : typeof m2 === "number" ? ((val2 = m2), (src2 = m2)) : null;
